...
const Layout: React.FC<LayoutProps> = ({ children, activeScreen, onNavigate, onOpenAnalytics }) => {
  return (
    <div className="max-w-md mx-auto min-h-screen flex flex-col bg-dark shadow-2xl border-x border-brand/10">
      <header className="p-4 border-b border-brand/20 flex flex-col gap-2 bg-black text-brand sticky top-0 z-10 shadow-lg">
        ...
        <AccountBar onOpenAnalytics={onOpenAnalytics} />
      </header>

      <main className="flex-1 overflow-y-auto p-4 bg-dark custom-scrollbar">{children}</main>
