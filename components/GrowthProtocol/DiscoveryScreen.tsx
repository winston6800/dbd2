...
import type { DiscoverablePerson, FollowedPerson, Group } from '../../lib/growth/types';
import { getDiscoveryList, addToDiscoveryList, decodeFollowProfileFromUrl, addFollowed, getFollowing } from '../../lib/growth/storage';
...
function buildDiscoverable(
  ...
  const links = getDiscoveryList(userId);
...
export const DiscoveryScreen: React.FC<DiscoveryScreenProps> = ({
  ...
  const discoverable = useMemo(
    () => buildDiscoverable(userId, groups, following, currentUserName),
    [userId, groups, following, currentUserName]
  );
  ...
  const handleFollow = (person: DiscoverablePerson) => {
    ...
    addFollowed(userId, { id, name: person.name, userState: person.userState });
  ...
  const handleAddLink = () => {
    ...
    if (!trimmed) return;
    addToDiscoveryList(userId, trimmed);
    setLinkInput('');
