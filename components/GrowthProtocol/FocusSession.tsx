...
import { Play, Pause, Minus, Plus, Sparkles } from 'lucide-react';
...
/**
 * Focus mode's content — swapped in for Growth Terminal's loop counter when
 * the Focus Timer badge is tapped (see BaseHub's `mode` state). The timer
 * itself (lifted up in BaseHub via useFocusTimer) keeps running even while
 * viewing Counter mode; switching back here doesn't stop it. "Cancel
 * session" is the only way to actually discard progress.
 */
...
}> = ({ timer, todayFocusMinutes, totalBits, onBackToCounter }) => {
  return (
    <div className="flex flex-col items-center space-y-5 text-center">
      {timer.justCompleted !== null ? (
        <>
          ...
          <div className="w-full bg-black/60 border border-brand/30 rounded-2xl py-5 space-y-1">
          ...
          <button
            onClick={() => { timer.dismissCompletion(); onBackToCounter(); }}
