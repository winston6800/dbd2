...
describe('storage', () => {
  ...
  describe('getUserState / saveUserState', () => {
    ...
    it('returns saved state after saveUserState', () => {
      ...
      custom.streak = 5;
      saveUserState(uid, custom);
      expect(getUserState(uid).streak).toBe(5);
    });

    it('keeps state scoped per user', () => {
      const custom = getDefaultUserState();
      custom.streak = 5;
      saveUserState(uid, custom);
      expect(getUserState('someone-else').streak).toBe(0);
    });
  });

  describe('getDisplayName / setDisplayName', () => {
    it('returns "You" when not set', () => {
      expect(getDisplayName(uid)).toBe('You');
    });

    it('returns saved name after setDisplayName', () => {
      setDisplayName(uid, 'Alex');
      expect(getDisplayName(uid)).toBe('Alex');
    });
  });
  ...
  describe('createGroup / getGroups', () => {
    it('creates group and persists to storage', () => {
      const state = getDefaultUserState();
      const group = createGroup(uid, 'My Squad', 'You', state);
  ...
  describe('updateGroup', () => {
    it('updates group members', () => {
      const state = getDefaultUserState();
      const group = createGroup(uid, 'Squad', 'A', state);
      const newMember = { id: 'm_2', name: 'B', userState: state };
      updateGroup(uid, group.id, { members: [...group.members, newMember] });
